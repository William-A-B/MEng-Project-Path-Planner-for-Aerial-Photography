...
EPS = 20;
% Max width of grid-aligned area
strip_width = 100;
num_strips = floor(x_max / strip_width)
...
% Generates a random point within a predefined sweep pattern
q_rand_list = [];
for i = 1:(num_strips+1)
    if mod(i, 2) == 1
        for y = 0:50:y_max
            q_rand_list = [q_rand_list; (i-1)*strip_width, y];
        end
    else
        for y = y_max:-50:0
            q_rand_list = [q_rand_list; (i-1)*strip_width, y];
...
for i = 1:1:numNodes
    ...
    if (i+1) <= height(q_rand_list)
        q_rand = q_rand_list((i+1), :);
    else
        q_rand = q_goal.coord;
    end
    ...
    if noCollision(q_rand, q_near.coord, obstacle)
        ...
        if q_near.coord(2) == y_max || q_near.coord(2) == 0
            if mod(iter, 2) == 1
                dubConnObj = dubinsConnection(MinTurningRadius=55);
                startPose = [q_near.coord(1), q_near.coord(2), pi/2]
                goalPose = [q_new.coord(1), q_new.coord(2), 3*pi/2]
...
% % Alternative "forced" path
% for i = 1:1:numNodes
%     q_rand = q_rand_list(i, :);
%     % Plot point on figure
%     plot(q_rand(1), q_rand(2), 'x', 'Color',  [0 0.4470 0.7410])
% 
%      % Break if goal node is already reached
